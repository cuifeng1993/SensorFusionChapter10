...
public:
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if ( jacobians ) {
      ...
      if ( jacobians[0] ) {
        Eigen::Map<Eigen::Matrix<double, 15, 15, Eigen::RowMajor>> jaco_i(jacobians[0]);
        jaco_i.setZero();

        // a. residual, position:
        jaco_i.block<3,3>(INDEX_P, INDEX_P) = - R_i_inv;
        jaco_i.block<3,3>(INDEX_P, INDEX_R) = Sophus::SO3d::hat( 
          ori_i.inverse() * (pos_j - pos_i - vel_i * T_ + 0.5 * g_ * T_ *T_) 
        );
        jaco_i.block<3,3>(INDEX_P, INDEX_V) = - T_ * R_i_inv;
        jaco_i.block<3,3>(INDEX_P, INDEX_A) = - J_.block<3,3>(INDEX_P, INDEX_A);
        jaco_i.block<3,3>(INDEX_P, INDEX_G) = - J_.block<3,3>(INDEX_P, INDEX_G);

        // b. residual, orientation:
        jaco_i.block<3, 3>(INDEX_R, INDEX_R) = -J_r_inv*(ori_j.inverse()*ori_i).matrix();
        jaco_i.block<3,3>(INDEX_R, INDEX_G) = - J_r_inv * ori_ij.matrix().inverse() *J_.block<3,3>(INDEX_R, INDEX_G);

        // c. residual, velocity:
        jaco_i.block<3,3>(INDEX_V, INDEX_R) = Sophus::SO3d::hat(
          ori_i.inverse() * (vel_j - vel_i + g_ * T_ )
        );
        jaco_i.block<3,3>(INDEX_V, INDEX_V) = - R_i_inv;
        jaco_i.block<3,3>(INDEX_V, INDEX_A) = - J_.block<3,3>(INDEX_V, INDEX_A);
        jaco_i.block<3,3>(INDEX_V, INDEX_G) = - J_.block<3,3>(INDEX_V, INDEX_G);

        // d. residual, bias accel:
        jaco_i.block<3,3>(INDEX_A, INDEX_A) = - Eigen::Matrix3d::Identity();

        // d. residual, bias accel:
        jaco_i.block<3,3>(INDEX_G, INDEX_G) = - Eigen::Matrix3d::Identity();

        jaco_i = sqrt_info * jaco_i;
      }

      if ( jacobians[1] ) {
        Eigen::Map<Eigen::Matrix<double, 15, 15, Eigen::RowMajor>> jaco_j(jacobians[1]);
        jaco_j.setZero();
        ...
        // b. residual, orientation:
        jaco_j.block<3,3>(INDEX_R, INDEX_R) = J_r_inv;

        // c. residual, velocity:
        jaco_j.block<3,3>(INDEX_V, INDEX_V) = R_i_inv;

        // d. residual, bias accel:
        jaco_j.block<3,3>(INDEX_A, INDEX_A) = Eigen::Matrix3d::Identity();

        // d. residual, bias accel:
        jaco_j.block<3,3>(INDEX_G, INDEX_G) = Eigen::Matrix3d::Identity();
