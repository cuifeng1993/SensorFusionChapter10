...
public:
  ...
  void Marginalize(
    ...
    const Eigen::MatrixXd &b_m = b_.block<15,1>(INDEX_M, 0);
    const Eigen::MatrixXd &b_r = b_.block<15,1>(INDEX_R, 0);
  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {	
    ...
    if ( jacobians ) {
      if ( jacobians[0] ) {
        // implement computing:
        Eigen::Map<Eigen::Matrix<double, 15, 15, Eigen::RowMajor>> jaco(jacobians[0]);
        jaco = J_;
      }
    }
