  ...
  virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
    ...
    if ( jacobians ) {
      if ( jacobians[0] ) {
        ...
        Eigen::Map<Eigen::Matrix<double, 6, 15, Eigen::RowMajor>> jaco( jacobians[0] );
        jaco.setZero();
        jaco.block<3,3>(INDEX_P,INDEX_P) = Eigen::Matrix3d::Identity();
        jaco.block<3,3>(INDEX_R,INDEX_R) =  JacobianRInv( residual.block<3,1>(INDEX_R,0) ) * ori_prior.matrix();
